		...
		try {
			...
			if (needed.size() > 0) {
				...
				if (!input.toLowerCase().equals("y")) {
					...
					FileDownloader down = new FileDownloader(URLs, outs, stringSet);
					down.run();
				}
			}
			PackageEntry theActual = this.getPackage(name);
			File OUT = new File(System.getProperty("user.dir") + File.separator + "run" + File.separator + "plugins"
					+ File.separator + theActual.getName() + ".jar");
			if (!OUT.exists() || update) {
				URL provider = new URL(theActual.getReleases().get(0).getFileURL());
				String info = "Downloading file: " + theActual.getReleases().get(0).getFileURL();
				FileDownloader FD = new FileDownloader(provider.toString(), OUT, info);
				FD.run();
			}
		} catch (Exception e) {
